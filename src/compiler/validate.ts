...
const METHODS = new Set<HttpMethod>(['ALL', 'GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']);
const AUTH_MODES = new Set<AuthMode>(['none', 'cloudflare-access', 'external-jwt']);
...
export function parseDeployIr(value: unknown): DeployIr {
  const root = record(value, 'IR');
  if (root.version !== DEPLOY_IR_VERSION) throw new Error(`IR.version must be ${DEPLOY_IR_VERSION}.`);
  const name = string(root.name, 'IR.name');
  const auth = string(root.auth, 'IR.auth') as AuthMode;
  if (!AUTH_MODES.has(auth)) throw new Error(`Unsupported auth mode: ${auth}`);
...
function parseRoute(value: unknown, index: number): RouteIr {
  const route = record(value, `IR.routes[${index}]`);
  const id = string(route.id, `IR.routes[${index}].id`);
  const method = string(route.method, `route ${id}.method`).toUpperCase() as HttpMethod;
  if (!METHODS.has(method)) throw new Error(`Unsupported method in route ${id}: ${method}`);
  const path = string(route.path, `route ${id}.path`);
  validatePath(path, id);
  const auth = string(route.auth, `route ${id}.auth`);
  if (auth !== 'public' && auth !== 'required') throw new Error(`Invalid auth guard in route ${id}.`);
  if (!Array.isArray(route.actions) || route.actions.length === 0) {
    throw new Error(`Route ${id} must contain at least one action.`);
  }
  const actions = route.actions.map((action, actionIndex) =>
    parseAction(action, `route ${id}.actions[${actionIndex}]`)
  );
...
function parseAction(value: unknown, location: string): ActionIr {
  ...
  if (kind === 'remove-header') return {kind, name: headerName(action.name, location)};
  ...
  if (kind === 'clear-handler-variables') return {kind};
  if (kind === 'respond') {
...
function parseExpression(value: unknown, location: string): ExpressionIr {
  const expression = record(value, location);
  const kind = string(expression.kind, `${location}.kind`);
  if (kind === 'literal') return {kind, value: string(expression.value, `${location}.value`)};
  if (kind === 'request') {
    ...
    return {kind, source: source as Extract<ExpressionIr, {kind: 'request'}>['source']};
  }
  ...
  if (kind === 'handler-variable-names') return {kind};
  if (kind === 'result') return {kind, name: resultName(expression.name, location)};
...
function headerName(value: unknown, location: string): string {
...
function collection(value: unknown, location: string): string {
  const name = string(value, `${location}.collection`);
  if (!COLLECTION_NAME.test(name)) throw new Error(`Invalid collection name at ${location}: ${name}`);
  return name;
}

function resultName(value: unknown, location: string): string {
...
function validateResultBindings(actions: ActionIr[], routeId: string): void {
  const available = new Set<string>();
  for (const action of actions) {
    for (const expression of actionExpressions(action)) validateExpressionResults(expression, available, routeId);
    if ('result' in action) {
...
function actionExpressions(action: ActionIr): ExpressionIr[] {
  if (action.kind === 'set-header') return [action.value];
  if (action.kind === 'set-handler-variable' || action.kind === 'change-handler-variable') {
    return [action.name, action.value];
  }
  if (action.kind === 'delete-handler-variable') return [action.name];
  if (action.kind === 'respond') return [action.body];
  if (action.kind === 'record-create') return [action.data];
  if (action.kind === 'record-get' || action.kind === 'record-delete') return [action.id];
  return [];
}

function validateExpressionResults(
