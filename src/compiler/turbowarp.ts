...
const METHODS = new Set<HttpMethod>(['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']);
...
export function compileTurboWarpProject(value: unknown): CompileResult {
  const project = requireObject(value, 'TurboWarp project') as ScratchProject;
  ...
  for (const rawTarget of project.targets) {
    const target = requireObject(rawTarget, 'project target') as ScratchTarget;
    ...
    const hats = Object.entries(blocks).filter(
      ([, block]) => block.opcode === HAT_OPCODE && block.topLevel === true
    );
  ...
  const ir: DeployIr = {version: DEPLOY_IR_VERSION, name: projectName, auth: 'none', routes};
...
function readMethodCondition(blocks: BlockMap, id: string): HttpMethod | null {
  ...
  for (const [reporterInput, literalInput] of pairs) {
    ...
    if (reporter?.opcode !== `${EXTENSION_PREFIX}currentHttpMethod`) continue;
    const method = literalValue(literalInput).toUpperCase() as HttpMethod;
...
function compileExpression(
  ...
  if (reporterId === null) return {kind: 'literal', value: literalValue(input)};
  ...
  const simpleSources: Record<string, Extract<ExpressionIr, {kind: 'request'}>['source']> = {
  ...
  if (source) return {kind: 'request', source};

  const namedSources: Record<string, Extract<ExpressionIr, {kind: 'request-value'}>['source']> = {
...
function rejectDuplicateRoutes(routes: RouteIr[], diagnostics: CompilerDiagnostic[]): void {
...
function requireBlockMap(value: unknown, target: string): BlockMap {
  return requireObject(value, `blocks for target ${target}`) as BlockMap;
}
...
function error(code: string, message: string, target: string, blockIdValue: string): CompilerDiagnostic {
