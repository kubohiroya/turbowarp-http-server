...
function parseConfig(config: unknown): {recordDatabaseBinding: string} | {diagnostic: PipelineDiagnostic} {
  if (config === undefined || config === null) return {recordDatabaseBinding: 'DB'};
  if (typeof config !== 'object' || Array.isArray(config)) return {diagnostic: configDiagnostic()};
  const record = config as Record<string, unknown>;
  if (Object.keys(record).some((key) => key !== 'recordDatabaseBinding')) return {diagnostic: configDiagnostic()};
  const value = record.recordDatabaseBinding ?? 'DB';
  if (typeof value !== 'string' || !/^[A-Z][A-Z0-9_]*$/u.test(value)) return {diagnostic: configDiagnostic()};
...
function cloudflareTypes(): string {
