...
export const firebaseFunctionsAdapter: PlatformAdapter = {
  ...
  plan({ir, requirements, config}) {
    const unsupported = requirements.keys.filter(
      (requirement) => !firebaseFunctionsAdapter.capabilities().keys.includes(requirement)
    );
...
function parseConfig(config: unknown): FirebaseConfig | {diagnostic: PipelineDiagnostic} {
  ...
  if (config === undefined || config === null) return defaults;
...
function relocateCore(core: GeneratedCore): GeneratedProject {
  return Object.fromEntries(
    Object.entries(core.files).map(([path, source]) => [
...
function functionsPackage(ir: DeployIrV2): string {
...
function functionsTsconfig(): string {
  return `${JSON.stringify(
    {
      compilerOptions: {
        target: 'ES2022',
...
function denyAllRules(service: 'cloud.firestore' | 'firebase.storage'): string {
...
function generatedReadme(plan: PlatformPlan): string {
  const bucketEnvironment = plan.bindings.objectBucketEnv;
