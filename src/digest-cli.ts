...
import {
  ...
  validateField,
  writeHtdigestFile
} from './auth/digest-file.js';

interface DigestCliOptions {
  command: 'init' | 'add' | 'passwd' | 'delete' | 'help';
  file?: string;
  username?: string;
  realm?: string;
  password?: string;
...
async function main(args: readonly string[]): Promise<void> {
  ...
  if (options.command === 'passwd') {
    ...
    if (!updated) throw new Error(`User ${username} was not found.`);
...
function parseArgs(args: readonly string[]): DigestCliOptions {
  ...
  const options: DigestCliOptions = {command};
