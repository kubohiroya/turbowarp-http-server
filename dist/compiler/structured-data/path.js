export function parseStructuredDataPath(input) {
    if (!input.startsWith('$'))
    ...
    while (cursor < input.length) {
        if (input[cursor] === '.') {
            ...
            if (match === null)
                throw new Error('Invalid identifier path segment.');
            segments.push({ kind: 'key', value: match[0] });
            cursor += match[0].length + 1;
            continue;
        }
        if (input[cursor] !== '[')
            throw new Error('Expected a bracket path segment.');
        cursor += 1;
        if (input[cursor] === '"') {
            ...
            if (typeof key !== 'string' || input[end] !== ']')
                throw new Error('Invalid quoted path key.');
        ...
        if (match === null)
            throw new Error('Invalid array index path segment.');
        const index = Number(match[1]);
        if (!Number.isSafeInteger(index))
            throw new Error('Array index is not a safe integer.');
        segments.push({ kind: 'index', value: index });
        cursor += match[0].length;
    }
    return segments;
}
function scanJsonString(path, start) {
    let escaped = false;
