...
function parseBinaryRef(value, location) {
    ...
    const byteLength = descriptor.byteLength === undefined ? undefined : integer(descriptor.byteLength, `${location}.byteLength`);
    if (byteLength !== undefined && byteLength < 0)
        throw new Error(`${location}.byteLength must not be negative.`);
    ...
    return optional(optional(optional({ namespace, key }, 'mediaType', mediaType), 'byteLength', byteLength), 'sha256', sha256);
}
