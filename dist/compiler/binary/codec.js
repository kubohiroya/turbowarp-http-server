...
export function validateBinaryMetadata(metadata) {
...
export function encodeContentDisposition(disposition) {
    if (disposition.kind === 'inline')
        return 'inline';
    if (disposition.filename.length < 1 ||
    ...
    return `attachment; filename*=UTF-8''${encodeURIComponent(disposition.filename).replace(/['()*]/gu, (value) => `%${value.codePointAt(0).toString(16).toUpperCase()}`)}`;
}
