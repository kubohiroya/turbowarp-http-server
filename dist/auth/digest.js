...
const QOP = 'auth';
const ALGORITHM = 'MD5';
export function createHa1(username, realm, password) {
    return md5(`${username}:${realm}:${password}`);
}
export function createDigestChallenge(options) {
...
export async function verifyDigestAuth(request, method, options) {
    ...
    if (!ha1 || !/^[0-9a-f]{32}$/i.test(ha1))
        return { ok: false, challenge };
    const requestUrl = new URL(request.url);
    if (uri !== `${requestUrl.pathname}${requestUrl.search}`)
        return { ok: false, challenge };
    const ha2 = md5(`${method.toUpperCase()}:${uri}`);
    const expected = params.qop === QOP && params.nc && params.cnonce
        ? md5(`${ha1}:${nonce}:${params.nc}:${params.cnonce}:${QOP}:${ha2}`)
        : md5(`${ha1}:${nonce}:${ha2}`);
    if (!safeEqualHex(response, expected))
        return { ok: false, challenge };
    return { ok: true, username };
}
export function parseDigestAuthorization(value) {
    const params = {};
    let index = 0;
...
function verifyNonce(nonce, secret, maxAgeMs = DEFAULT_NONCE_MAX_AGE_MS) {
    try {
        ...
        if (!timestampText || !randomText || !signature)
            return false;
...
function defaultNonceSecret() {
    globalThis.__turbowarpHttpDigestNonceSecret ?? (globalThis.__turbowarpHttpDigestNonceSecret = randomBytes(32).toString('base64url'));
